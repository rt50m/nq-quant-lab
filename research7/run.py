...
import argparse,json,os,sys,hashlib,itertools,math
...
def _json_clean(x):
    # GitHub verification data contains numpy scalars, dataclasses and, in
    # degenerate correlation/stat cases, non-finite floats. Normalize all of
    # them before strict JSON serialization so diagnostics never crash the run.
    if is_dataclass(x):
        x=asdict(x)
...
def atomic_json(path,obj):
    ...
    tmp.write_text(json.dumps(_json_clean(obj),indent=2,allow_nan=False)+'\n')
    os.replace(tmp,path)
...
def build(prepared,out):
    ...
    for ci,can in enumerate(c['candidates']):
        if can['source']=='R1':
            if r1state is None:r1state=common_r1_state(prepared)
...
def load_sleeves(sleeves):
    s=Path(sleeves);obj=json.load(open(s/'logical-options.json'));logical=obj['options'];phys=obj['physical'];daily=np.load(s/'logical-daily.npy')
    # np.load(.npz) is lazy. Keeping the NpzFile and repeatedly indexing it
    # re-decompresses large arrays for every exact finalist. Materialize once.
    with np.load(s/'physical-paths.npz') as z:
        paths={k:z[k] for k in ('delta','adverse','exposure')}
    cand=json.load(open(s/'candidates.json'));dates=np.array(cand['dates']);return logical,phys,daily,paths,cand,dates
...
def search(sleeves,out,index,count,samples):
    ...
    choices=np.vstack([x for x in allc if len(x)]);seen={};seen_source={};B=2000
    for start in range(0,len(choices),B):
        ...
        # Daily MDD is a necessary (but not sufficient) condition for the exact
        # intraminute MDD cap. Do not waste exact-finalist slots on portfolios
        # that already violate $2k at daily resolution.
        soft_ok=soft<=c['search']['approx_exposure_soft_cap']
        evidence_good=np.where(soft_ok & (msel['net']>0) & (msel['mdd']>-c['objective']['max_drawdown']) & (msel['net_2023']>0) & (msel['net_2024']>0))[0]
        oracle_good=np.where(soft_ok & (mfull['net']>0) & (mfull['mdd']>-c['objective']['max_drawdown']))[0]
        aa=np.array([],dtype=int);bb=np.array([],dtype=int)
        if len(evidence_good):aa=evidence_good[np.argsort(score[evidence_good])[-min(140,len(evidence_good)):]]
        if len(oracle_good):
            oracle=mfull['net']/np.maximum(100,np.abs(mfull['mdd']));bb=oracle_good[np.argsort(oracle[oracle_good])[-min(100,len(oracle_good)):]]
        for j in aa:
            k=key_choice(ch[j]);seen[k]=ch[j].copy();seen_source.setdefault(k,set()).add('EVIDENCE_2023_2024')
        for j in bb:
            k=key_choice(ch[j]);seen[k]=ch[j].copy();seen_source.setdefault(k,set()).add('ORACLE_2023_2025')
    finalists=list(seen.values())
    # Reserve exact-finalist capacity separately for evidence-only and oracle
    # rankings. This prevents 2025 oracle ranking from crowding out a portfolio
    # that would have survived on 2023+2024 evidence alone.
    ranked=[]
    for ch in finalists:
        dp=sum((daily[int(o)] for o in ch),np.zeros(len(dates),dtype=float));sel=dp[years<=2024];ms=mdd_from_daily(sel)
        escore=sel.sum()/max(100,abs(ms)) if sel.sum()>0 and ms>-c['objective']['max_drawdown'] else -1e99
        oscore=dp.sum()/max(100,abs(mdd_from_daily(dp))) if dp.sum()>0 and mdd_from_daily(dp)>-c['objective']['max_drawdown'] else -1e99
        ranked.append((escore,oscore,ch))
    limit=c['search']['exact_finalists_per_shard'];ne=int(round(limit*.60));no=limit-ne
    ev=sorted(ranked,key=lambda z:z[0],reverse=True)[:ne];ora=sorted(ranked,key=lambda z:z[1],reverse=True)[:no]
    keep={key_choice(z[2]):z for z in ev+ora};ranked=list(keep.values())
    rows=[]
    for _,__,ch in ranked:
        st,meta,contrib=exact_one(ch,logical,phys,paths,dates,cand)
        sel_daily=sum((daily[int(o)] for o in ch),np.zeros(len(dates),dtype=float))[years<=2024];sel_mdd=mdd_from_daily(sel_daily);sel_net=float(sel_daily.sum());n23=st['net_2023'];n24=st['net_2024']
        evidence=(min(n23,n24)>0) and sel_net>0 and sel_mdd>-c['objective']['max_drawdown']
        ck=key_choice(ch)
        row={'choice':list(map(int,ch)),'labels':[logical[int(o)]['label'] for o in ch],'strategies':[logical[int(o)]['candidate_name'] for o in ch],'stats':st,'meta':meta,'contributions':contrib,'pruning_sources':sorted(seen_source.get(ck,set())),
...
def aggregate(sleeves,parts,out,count):
    ...
    selection_valid=[r for r in rows if r['selection_score']>-1e98]
    disciplined=sorted(selection_valid,key=lambda r:(r['selection_score'],r['stats']['net_profit']),reverse=True)
    feasible=[r for r in rows if r['stats']['max_drawdown']>-c['objective']['max_drawdown'] and r['stats']['max_exposure_equiv_mnq']<=c['objective']['max_equivalent_mnq_exposure']]
    disciplined_feasible=sorted([r for r in feasible if r['selection_score']>-1e98],key=lambda r:(r['selection_score'],r['stats']['net_profit']),reverse=True)
    oracle=sorted(feasible,key=lambda r:(r['stats']['net_profit'],r['stats']['profit_to_dd']),reverse=True)
    passes=[r for r in rows if r['pass_scale']];strict=[r for r in rows if r['pass_prop_strict']];cred=[r for r in rows if r['credible']]
    pass_rows=sorted(passes,key=lambda r:(r['stats']['net_profit'],r['stats']['profit_to_dd']),reverse=True)
    atomic_json(Path(out)/'report.json',{'unique_exact_portfolios':len(rows),'scale_passes':len(passes),'prop_strict_passes':len(strict),'credible_passes':len(cred),'hard_scale_passes':pass_rows[:100],'best_disciplined':disciplined[:100],'best_disciplined_feasible':disciplined_feasible[:100],'best_oracle':oracle[:100]})
    bestd=disciplined_feasible[0] if disciplined_feasible else None;besto=oracle[0] if oracle else None
    lines=['# Research 7 — Multi-Strategy Hedge / Portfolio Optimization','',f'Exact minute-level finalists evaluated: **{len(rows):,}**.','',
           ...
           '## Disciplined feasible leaderboard','',
           'Allocations must first satisfy the 2023+2024 selection discipline. Among portfolios that later remain inside the full-period hard DD/exposure constraints, ranking still uses the frozen 2023+2024 evidence score; 2025 never chooses the order.','',
           '| Rank | Full net | MDD | P/DD | PF | Trades | 2023 | 2024 | 2025 | Worst day | Max exp | Active | Allocation |','|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|']
    for i,r in enumerate(disciplined_feasible[:25],1):
    ...
    if pass_rows:
        lines += ['','## Hard scale passes','', '| Net | MDD | P/DD | PF | Trades | 2023 | 2024 | 2025 | Selection score | Pruned by | Allocation |','|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|']
        for r in pass_rows[:20]:
            st=r['stats'];alloc='; '.join(f"{n}: {lab}" for n,lab in zip(r['strategies'],r['labels']) if lab!='OFF');src='+'.join(r.get('pruning_sources',[]))
            lines.append(f"| {st['net_profit']:,.0f} | {st['max_drawdown']:,.0f} | {st['profit_to_dd']:.1f} | {st['profit_factor']:.2f} | {r['meta']['portfolio_trades']} | {st['net_2023']:,.0f} | {st['net_2024']:,.0f} | {st['net_2025']:,.0f} | {r['selection_score']:.2f} | {src} | {alloc} |")
