...
def build_physical_path(trades,a,mode,spec,ndays):
    ...
    for i,t in enumerate(trades):
        ...
        for k in range(entry_i,exit_i+1):
            ...
            wp=min(0.0,wp)
            # Recompute the adverse envelope from each actual minute. Do not
            # smear the trade's eventual worst excursion backward across all
            # earlier minutes; that would use future information and overstate
            # overlap drawdown.
            val=wp*pvv*q-fee
