...
class TestR7(unittest.TestCase):
    ...
    def test_numpy_str_timestamp_boundary(self):
        import pandas as pd
        ds=np.str_('2023-01-03')
        self.assertEqual(str(pd.Timestamp(str(ds)).date()),'2023-01-03')

    def test_atomic_json_numpy_and_nonfinite(self):
        # Explicitly load this research directory's run.py by file path.
        # A generic `import run` can resolve an unrelated installed module.
        r7run=load_spec('r7_run_test_target',Path(__file__).with_name('run.py'))
        with tempfile.TemporaryDirectory() as td:
            p=Path(td)/'x.json'
            r7run.atomic_json(p,{'n':np.int64(3),'x':np.float64(1.5),'bad':np.nan,'a':np.array([1,2])})
            z=json.loads(p.read_text())
            self.assertEqual(z['n'],3);self.assertIsNone(z['bad']);self.assertEqual(z['a'],[1,2])
